#!/bin/sh
# Phase 2 回归: 先对账 TD 工程登记, 再跑三个纯 RTL testbench
...
vvp sim/out/tb_ycbcr.vvp | grep -E "值域|比对像素|PASS|FAIL"
...
vvp sim/out/tb_fb.vvp | grep -E "统计|提交|重叠|PASS|FAIL"
...
vvp sim/out/tb_dp.vvp | grep -E "统计|检查|覆盖|错位|撞拍|PASS|FAIL"
